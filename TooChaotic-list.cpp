...
void addCulprits(list<int> culprits, int culprit)
{
...
int findCulprits(list<int> culprits, int suspect) {

    int cnt = 0;

    //for(int i = 0; i < culprits.size(); i++)
    for (list<int>::iterator it = culprits.begin(); it != culprits.end(); it++)
    {
        if(*it > suspect)
        {
            break;
